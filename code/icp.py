...
def find_projective_correspondence(source_points,
    ...
    valid_bounds = (target_us >= 0) & (target_us < w) & (target_vs >= 0) & (
        target_vs < h) & (target_ds > 0)
    mask = valid_bounds
    ...
    # q, nq, p , np
    q = target_points
    n_q = target_normal_map[target_vs, target_us]
    p = T_source_points
    n_p = source_normals[mask]
    ...
    # Normal Threshold from the paper is 20 degrees
    angle_threshold_cos = np.cos(np.radians(20))
    # dot(q,p) = cos(theta) where theta is the angle between vectors
    angle_dif = np.sum(n_q * n_p, axis=1)
    valid_angle = angle_dif > angle_threshold_cos

    
    # Filter for valid distance and angle
...
def build_linear_system(source_points, target_points, target_normals, T):
    ...
    A[:, 0:3] = np.cross(p_prime, n_q, axis=1)
    A[:, 3:6] = n_q
...
def solve(A, b):
    ...
    # return np.zeros((6, ))
    # cholesky solve and cholesky factor
    # print(np.size(A.T@A))
